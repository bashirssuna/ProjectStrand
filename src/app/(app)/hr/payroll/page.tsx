...
export default async function PayrollPage({ searchParams }: { searchParams: Promise<{ comp?: string; run?: string; finalised?: string; err?: string }> }) {
  ...
  return (
    <div className="max-w-5xl">
      <PageHeader title="Payroll & compensation" subtitle="Pay components, the grant compensation model, monthly runs & payslips" actions={<div className="flex gap-2"><Link href="/hr/payroll/wizard" className="btn btn-sm btn-primary">PAYE wizard</Link><Link href="/hr/compensation" className="btn btn-sm">Compensation model</Link><Link href="/hr" className="btn btn-sm">← HR</Link></div>} />
