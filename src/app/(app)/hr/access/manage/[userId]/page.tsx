...
export default async function PersonAccessPage({ params, searchParams }: { params: Promise<{ userId: string }>; searchParams: Promise<{ saved?: string; err?: string }> }) {
  ...
  const { orgId, userId: meId } = await requireHrOrg();
  ...
  return (
    <div className="max-w-3xl">
      <PageHeader title={person.name} subtitle="Access & permissions" actions={<Link href="/hr/access/manage" className="btn btn-sm">← All people</Link>} />
