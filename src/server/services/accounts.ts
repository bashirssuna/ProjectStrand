...
export async function getUserOrg(userId: string): Promise<{ id: string; name: string; plan: string; status: string; trialEndsAt: string | null; subscriptionEndsAt: string | null; subscriptionTermMonths: number | null; isOrgAdmin: boolean; baseCurrency: string; displayCurrency: string | null } | null> {
  return one(
    `SELECT o.id, o.name, o.plan, o.status, o.trial_ends_at AS "trialEndsAt",
            o.subscription_ends_at AS "subscriptionEndsAt", o.subscription_term_months AS "subscriptionTermMonths",
