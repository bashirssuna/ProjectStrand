...
import { quotationGate, listQuotations } from "@/server/services/procurement";
import { PageHeader, SectionTitle, Field, Badge, StatusBadge, Empty } from "@/components/ui";
import { money, fmtDate } from "@/lib/format";
import { decidePurchaseRequestAction, createPOAction, addQuotationAction, selectQuotationAction, deleteQuotationAction, saveSingleSourceJustificationAction } from "@/app/actions";

export default async function PurchaseRequestDetail({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ saved?: string; err?: string }> }) {
  const { id } = await params;
  const { orgId } = await requireProcOrg();
  const sp = await searchParams;

  const pr = await one<{ id: string; number: string; title: string; status: string; estimatedTotal: number; currency: string; neededBy: string | null; justification: string | null; requestedByName: string | null; projectCode: string | null; projectId: string | null; lineCode: string | null; singleSource: string | null }>(
    `SELECT pr.id, pr.number, pr.title, pr.status, pr.estimated_total::float AS "estimatedTotal", pr.currency,
            ...
            p.code AS "projectCode", p.id AS "projectId", bl.code AS "lineCode"
  ...
  return (
    <div className="max-w-4xl">
      ...
      <form action={addQuotationAction} className="card p-4 grid sm:grid-cols-4 gap-3 items-end mb-5">
      ...
      {pr.status === "submitted" && (
        <div className="card p-4 flex flex-wrap gap-2 items-center">
          <span className="text-sm font-medium mr-2">Decision:</span>
          <form action={decidePurchaseRequestAction}><input type="hidden" name="requestId" value={id} /><button className="btn btn-sm btn-primary" name="decision" value="approved" type="submit">Approve</button></form>
          <form action={decidePurchaseRequestAction}><input type="hidden" name="requestId" value={id} /><button className="btn btn-sm" name="decision" value="rejected" type="submit" style={{ color: "var(--danger)", borderColor: "var(--danger)" }}>Reject</button></form>
          {gate.enforce && !gate.ok && !gate.hasSingleSource && <span className="text-xs" style={{ color: "var(--warn)" }}>Approval is blocked until quotations are met or a single-source justification is recorded.</span>}
        </div>
      )}
      {pr.status === "approved" && (
        vendors.length > 0 ? (
